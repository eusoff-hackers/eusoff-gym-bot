...
function register(content) {
  ...
  var chatID = content.message.from.id;
  Logger.log(chatID);
  ...
  if (data.split(' ')[0] === 'book-morn') {
    ...
    for (i = bookrow; i <= bookrow + 4; i++) {
      if (bookingdata[i][day] === room) {
        ...
        sendText(userID, 'Successfully booked ' + bookingdata[0][day] + ' ' + bookingdata[bookrow][0]);
        return;
      } else if (i === bookrow + 4) { // reach last cell and still hasnt inserted name
        sendText(userID, bookingdata[0][day] + ' ' + bookingdata[bookrow][0] + ' slot is full, try another one');
        return;
      }
    }
    ...
    for (i = bookrow; i <= bookrow + 4; i++) {
      if (bookingdata[i][day] === room) {
        ...
        bookingsheet.getRange(i + 1, day + 1).setValue(room); // indexing for actual data starts from 1
        sendText(userID, 'Successfully booked ' + bookingdata[1][day] + ' ' + bookingdata[bookrow][1]);
        return;
      } else if (i === bookrow + 4) { // reach last cell and still hasnt inserted name
        sendText(userID, bookingdata[0][day] + ' ' + bookingdata[bookrow][0] + ' slot is full, try another one');
        return;
