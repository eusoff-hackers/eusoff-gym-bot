function weekRefresh() {
  ...
  // Date manipulation
  var today = new Date();
  var lastweek = Utilities.formatDate(
    new Date(today.getTime() - 24 * 60 * 60 * 1000 * 6),
    tz,
    'MM/dd'
  );
  var monday =
  ...
  var thursday =
    'Thursday ' +
    Utilities.formatDate(
      new Date(today.getTime() + 24 * 60 * 60 * 1000 * 4),
      tz,
      'MM/dd'
    );
  var friday =
    'Friday ' +
    Utilities.formatDate(
      new Date(today.getTime() + 24 * 60 * 60 * 1000 * 5),
      tz,
      'MM/dd'
    );
  var saturday =
  ...
  var sunday =
    'Sunday ' +
    Utilities.formatDate(
      new Date(today.getTime() + 24 * 60 * 60 * 1000 * 7),
      tz,
      'MM/dd'
    );
  var nextMonday =
  ...
  destinationSheet.setName('Week: ' + lastweek);

  // Reseting the Current Week
  originSheet.getRange('B2:H92').clearContent();
